...
public:
    ...
    void startToBuildData() {
        ...
        memset(mBuffer, 0, static_cast<size_t>(CompressIndex::NXTBUFFER));
        mNextBuffer = static_cast<int32_t>(CompressIndex::NXTBUFFER);
...
private:
    ...
    uint8_t* getResourceTransferBuffer(int id, int indexStart, int indexEnd) {
        if (indexStart <= 0 || indexEnd <= 0 || (indexEnd - indexStart <= 0)) {
            return nullptr;
        }
    ...
    uint8_t* getResourceByAddr(int addr) {
        if (addr < static_cast<uint32_t>(CompressIndex::NXTBUFFER)) {
            return nullptr;
        }
